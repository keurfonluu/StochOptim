...
module stochoptim
  ...
  type Evolutionary
    ...
    integer(kind = IPRE) :: n_dim, popsize = 10, max_iter = 100, verbose = 0
    real(kind = RPRE) :: eps1 = 1e-8, eps2 = 1e-8
    logical :: constrain = .true., snap = .false.
    character(len = 5) :: solver, strategy
...
contains
  ...
  subroutine de(self, CR, F, strategy, xstart)
    ...
    real(kind = RPRE) :: gfit
    real(kind = RPRE), dimension(:), allocatable :: pfit, pbestfit, gbest, xopt
    ...
    do while ( .not. converge )
      ...
      if ( norm(gbest - X(gbidx,:)) .lt. self % eps1 &
        .and. pbestfit(gbidx) .lt. self % eps2 ) then
        converge = .true.
      ...
      else if ( pbestfit(gbidx) .lt. self % eps2 ) then
        converge = .true.
        xopt = self % unstandardize(X(gbidx,:))
        gfit = pbestfit(gbidx)
        self % iflag = 1
